...
{
  M->ax=2.0*(SQR(q.r)+SQR(q.i))-1.0; M->bx=2.0*(q.i*q.j+q.r*q.k);       M->cx=2.0*(q.i*q.k-q.r*q.j);
  M->ay=2.0*(q.i*q.j-q.r*q.k);       M->by=2.0*(SQR(q.j)+SQR(q.r))-1.0; M->cy=2.0*(q.j*q.k+q.r*q.i);
  M->az=2.0*(q.i*q.k+q.r*q.j);       M->bz=2.0*(q.j*q.k-q.r*q.i);       M->cz=2.0*(SQR(q.r)+SQR(q.k))-1.0;
}
...
void ComputeQuaternions(void)
{
  ...
  for(j=0;j<NumberOfComponents;j++)
  {
    ...
    {
      ill=0;
      rtall=Components[j].Groups[k].InertiaVector.x+Components[j].Groups[k].InertiaVector.y+Components[j].Groups[k].InertiaVector.z;
      ...
      rotmin=MIN2(Components[j].Groups[k].InertiaVector.x,Components[j].Groups[k].InertiaVector.y);
      rotmin=MIN2(rotmin,Components[j].Groups[k].InertiaVector.z)/rotall;

      Components[j].Groups[k].rot_min=rotmin; 

      if(Components[j].Groups[k].InertiaVector.x/rotall<1.0e-5) ill++;
      if(Components[j].Groups[k].InertiaVector.y/rotall<1.0e-5) ill++;
      if(Components[j].Groups[k].InertiaVector.z/rotall<1.0e-5) ill++;
      ...
      switch(ill)
      {
        ...
        case 0:
          ...
          do
          {
            ...
            {
              i3++;
              A=Components[j].Groups[k].Atoms[i1-1];
              B=Components[j].Groups[k].Atoms[i2-1];
              C=Components[j].Groups[k].Atoms[i3-1];
            ...
            while((fabs(det)<dettest)&&(i3<Components[j].NumberOfAtoms));
          }
          while((fabs(det)<dettest)&&(i2<Components[j].NumberOfAtoms-1));
          Components[j].Groups[k].RotationalMatrix=bb;
          break;
        // linear molecule
        case 1:
          i1=1;
      ...
      rotall=1.0/MAX2((REAL)1.0e-5,Components[j].Groups[k].InertiaVector.x+Components[j].Groups[k].InertiaVector.y+
               Components[j].Groups[k].InertiaVector.z);
...
// compute the quaternions for the rigid units from the postions of the atoms
void ComputeQuaternionAdsorbate(int m)
{
  ...
  for(k=0;k<Components[Type].NumberOfGroups;k++)
  {
    if(Components[Type].Groups[k].Rigid)
    {
      ...
      bb=Components[Type].Groups[k].RotationalMatrix;
