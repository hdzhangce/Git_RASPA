...
void ReadPseudoAtomsDefinitions(void)
{
  ...
  for(i=0;i<NumberOfPseudoAtoms;i++)
  {
    ...
    sscanf(line,"%s %s %s %s %s %lf %lf %lf %lf %lf %d %lf %s %d",
    ...
    PseudoAtoms[i].Polarization=(REAL)(temp3/COULOMBIC_CONVERSION_FACTOR);
    ...
    if(fabs(PseudoAtoms[i].Polarization)<1e-10)
...
void ReadComponentDefinition(int comp)
{
  ...
  for(i=0;i<Components[comp].NumberOfGroups;i++)
  {
    ...
    ReadLine(line,1024,FilePtr);
    sscanf(line,"%d",&Components[comp].Groups[i].NumberOfGroupAtoms);    // read NumberOfGroups elements
