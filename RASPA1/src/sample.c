      ...
      for(i=0;i<NumberOfAdsorbateMolecules[CurrentSystem];i++)
      {
        for(k=0;k<Adsorbates[CurrentSystem][i].NumberOfAtoms;k++)
        {
          ...
          if(PseudoAtoms[TypeA].PrintToPDB)
          {
            ...
            for(j=0;j<Framework[CurrentSystem].TotalNumberOfAtoms;j++)
            {
              TypeB=Framework[CurrentSystem].Atoms[0][j].Type;
              if(PseudoAtoms[TypeB].PrintToPDB)
              {
                posB=Framework[CurrentSystem].Atoms[0][j].Position;
                dr.x=posA.x-posB.x;
                dr.y=posA.y-posB.y;
                dr.z=posA.z-posB.z;
    ...
    case PRINT:
     ...
     // make the output directory
      ...
      for(i=0;i<NumberOfComponents;i++)
      {
        ...
        // restrict the size of the density grid to 150x150x150
